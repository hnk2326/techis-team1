...
return [
    ...
    /* [月森]
    https://macocci7.net/blog/2024/05/08/%E3%80%90laravel11%E3%80%91%E6%97%A5%E6%9C%AC%E8%AA%9E%E5%8C%96%E8%A8%AD%E5%AE%9A/
    上記Urlのブログを参考に導入しています。
    フォームで受け取ったデータのバリデーションでエラーが起きた時のエラーメッセージです。
    （例）
        コントローラの中：　validate([ 'name' => 'required|max:15' ]);
            に対して５０文字の名前を登録しようとした場合に
        このファイルの中100行目くらい：　'max' => ['string' => ':attributeは、:max文字以下で指定してください。',]
            に当てはめて
        「nameは、15文字以下で指定してください。」
            というエラーメッセージが日本語で作られるようになっています。
        「nameは」という部分を「名前は」と表示させるためには、このファイル１７０行以降で指定します。

    ブログの方法とは別に、自分で一つずつメッセージを作ることもできるようです。
    「バリデーション　エラーメッセージ　日本語化」などで検索
    */
